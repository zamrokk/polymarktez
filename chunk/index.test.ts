...
const betPUSHRequest = new Request("tezos://fake/bet", {
  method: "PUSH",
  body: JSON.stringify({
    option: "trump",
    amount: 1,
...
const findBetRequest = (betId: string) =>
  new Request("tezos://fake/bet/" + betId, {
    method: "GET",
    headers: headers(alice),
  });

const getOddsRequest = (option: string, betAmount: number) =>
  new Request("tezos://fake/odds?option=" + option + "&amount=" + betAmount, {
    method: "GET",
    headers: headers(alice),
  });
...
const resultRequest = new Request("tezos://fake/result");

const resultOnRequest = (option: string, result: BET_RESULT) =>
  new Request("tezos://fake/result", {
    method: "POST",
    body: JSON.stringify({
      option,
      result,
